...
4. **`trust proxy = 1` non vérifié sur Render** (derrière Cloudflare) : si `req.ip` est l'IP du proxy, tous les limiteurs par IP (login 10/15 min, inscription 5/h, IA, PDF, entrée) partagent un seul compteur pour tous les utilisateurs. **Vérification en cours (2026-09-26) :** un point de contrôle **temporaire** `GET /api/_diagnostics/client-ip` ([app.js](backend/src/app.js)) renvoie à l'appelant `req.ip`, `X-Forwarded-For`, l'adresse de la socket et l'IP qu'Express retiendrait pour 0 à 4 sauts de proxy ; à comparer avec l'IP publique réelle de l'appelant. **⚠ À SUPPRIMER dès la conclusion** (et à ajuster `app.set("trust proxy", …)` si besoin).
