...
4. **`trust proxy = 1` faux sur Render (derrière Cloudflare)** — **confirmé en production (2026-09-26), corrigé, à re-vérifier en ligne après déploiement.** Mesure via un point de contrôle temporaire `GET /api/_diagnostics/client-ip`, appelé depuis une machine d'IP publique connue :
   ...
   - **⚠ Le point de contrôle temporaire existe encore** ([app.js](backend/src/app.js)) pour confirmer en production que `reqIp` est bien l'IP réelle après déploiement. **À SUPPRIMER ensuite.**
